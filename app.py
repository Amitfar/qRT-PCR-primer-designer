                    ...
                    for j in junction_list:
                        ...
                        for i in range(num_ret):
                            j_candidates.append({
                                ...
                                'f_hairpin': raw_res.get(f'PRIMER_LEFT_{i}_HAIRPIN_TH', 0), 'r_hairpin': raw_res.get(f'PRIMER_RIGHT_{i}_HAIRPIN_TH', 0),
                                'f_self': raw_res.get(f'PRIMER_LEFT_{i}_SELF_ANY_TH', 0), 'r_self': raw_res.get(f'PRIMER_RIGHT_{i}_SELF_ANY_TH', 0),
                                'cross_dimer': raw_res.get(f'PRIMER_PAIR_{i}_COMPL_ANY_TH', 0),
                ...
                if not final_candidates: 
                    ...
                    for idx, cand in enumerate(final_candidates):
                        ...
                        parsed_data.append({
                            ...
                            "F_Self (ΔG)": round(cand['f_self'], 2),
                            "F_Hairpin (ΔG)": round(cand['f_hairpin'], 2),
                            "F_BLAST": f_blast_sum,
                            "F_Tm (°C)": round(cand['forward_tm'], 1),
                            "Reverse_pos": f"R{cand['right_start']}",
                            "Reverse_Seq": cand['reverse_seq'],
                            "R_Self (ΔG)": round(cand['r_self'], 2),
                            "R_Hairpin (ΔG)": round(cand['r_hairpin'], 2),
                            "R_BLAST": r_blast_sum,
