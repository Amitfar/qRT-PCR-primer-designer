...
with st.sidebar:
    ...
    with st.expander("🎯 Smart Target Input"):
        st.write("""
        The **Target** field automatically detects your design strategy:
        * **Single Number (e.g. 452):** **Junction Mode**. Ensures the primer spans an exon-exon junction with a strict **7-bp anchor** on each side.
        * **Range (e.g. 300-450):** **ROI Mode**. Forces the entire amplicon to be designed **inside** this specific sequence range.
        * **Empty:** **Global Mode**. Searches the entire sequence for the best thermodynamic pairs.
        """)

    with st.expander("📦 Batch Processing"):
        st.write("""
        1. **Download Template:** Use the provided CSV to format your data.
        2. **Upload:** SPUD will process every row independently.
        3. **BLAST Strategy:** To save time, BLAST is only performed on the **Rank 1** candidate for each gene.
...
if mode == "Batch Upload (CSV/Excel)":
    ...
    with col_a: s_seq = st.text_area("Target Sequence (5' to 3'):", height=150)
    with col_b:
        s_id = st.text_input("Gene ID:", "MyGene")
        s_target = st.text_input("Target (Smart Input):", placeholder="e.g., 452 OR 300-450")
    if s_seq: genes_data.append({"Gene_ID": s_id, "Sequence": s_seq, "Target": s_target})
...
if st.button("🚀 Run SPUD Engine", type="primary") and genes_data:
    ...
    for idx, gene in enumerate(genes_data):
        ...
        try:
            ...
            for i in range(raw_res.get('PRIMER_PAIR_NUM_RETURNED', 0)):
                f_start, f_len = raw_res.get(f'PRIMER_LEFT_{i}')[0], raw_res.get(f'PRIMER_LEFT_{i}')[1]
                r_start, r_len = raw_res.get(f'PRIMER_RIGHT_{i}')[0], raw_res.get(f'PRIMER_RIGHT_{i}')[1]
                f_seq, r_seq = raw_res.get(f'PRIMER_LEFT_{i}_SEQUENCE'), raw_res.get(f'PRIMER_RIGHT_{i}_SEQUENCE')
                f_tm, r_tm = raw_res.get(f'PRIMER_LEFT_{i}_TM'), raw_res.get(f'PRIMER_RIGHT_{i}_TM')
                ...
                valid_candidates.append({
                    "Gene_ID": gene['Gene_ID'], "Forward_Seq": f_seq, "Reverse_Seq": r_seq,
                    "F_Tm": round(f_tm, 1), "R_Tm": round(r_tm, 1),
                    "Amp_Len": raw_res.get(f'PRIMER_PAIR_{i}_PRODUCT_SIZE'),
                    "Template_Fold": sec_struct_flag, "Penalty": raw_res.get(f'PRIMER_PAIR_{i}_PENALTY') + (50 if sec_struct_flag == "!" else 0)
                })
            
            valid_candidates.sort(key=lambda x: x['Penalty'])
            for rank, cand in enumerate(valid_candidates[:num_returns]):
                cand['Rank'], cand['Status'] = rank + 1, "Pending"
                all_results.append(cand)
            if not valid_candidates: all_results.append({"Gene_ID": gene['Gene_ID'], "Status": "No candidates found"})
        except Exception as e: all_results.append({"Gene_ID": gene['Gene_ID'], "Status": f"Error: {e}"})
        progress_bar.progress((idx + 1) / len(genes_data))

    # Final Audit
    status_text.text("Finalizing Quality Checks...")
    ...
    for cand in all_results:
        if cand.get('Status') == "Pending":
            cand['Tm_Flag'] = "Outlier" if abs(((cand['F_Tm'] + cand['R_Tm']) / 2.0) - global_mean_tm) > 1.5 else "OK"
            ...
            else: cand['Status'] = "Ready"

    status_text.text("Complete!")
    df_results = pd.DataFrame(all_results)
    st.dataframe(df_results, use_container_width=True)
    st.download_button(label="📥 Download Results (CSV)", data=df_results.to_csv(index=False).encode('utf-8'), file_name="SPUD_Results.csv", mime="text/csv")
